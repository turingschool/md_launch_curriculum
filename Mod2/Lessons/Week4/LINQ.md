...
`Where` is a method that takes a [lambda expression](/Mod2/Preparation/Week4/LINQ.md) as an argument.  In this case, the lambda expression is telling `Where` _how_ to filter the collection.  We want to look at each `name` and filter to only those names where `name[0] == 'C'` is true.
...
** Instructor Note ** Use the last question about `Distinct` to introduce the [LINQ method docs](https://learn.microsoft.com/en-us/dotnet/api/system.linq.enumerable.distinct?view=net-7.0)
